...
public class DefaultPersonServiceTests {
    ...
    public void testReconciliationResultExactMatch() throws ReconciliationException, SorPersonAlreadyExistsException {
        this.personService = new DefaultPersonService(personRepository, new MockReferenceRepository(), new MockDisclosureRecalculationStrategyRepository(), new NoOpIdentifierGenerator(), new MockReconciler(ReconciliationType.EXACT));
        this.personService.setPersonObjectFactory(this.objectFactory);
        ServiceExecutionResult<Person> result = this.personService.addPerson(reconciliationCriteria);
        assertNotNull(result);
        assertNotNull(result.getTargetObject());
    ...
    public void testAddPersonAndLinkWithBadPerson() throws SorPersonAlreadyExistsException {
        this.personService = new DefaultPersonService(this.personRepository, new MockReferenceRepository(), new MockDisclosureRecalculationStrategyRepository(), new NoOpIdentifierGenerator(), new MockReconciler(ReconciliationType.MAYBE));
        this.personService.setPersonObjectFactory(this.objectFactory);
